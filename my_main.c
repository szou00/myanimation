...
void first_pass() {
  ...
  int foundf = 0; //keeps track of whether frames is found
  int foundb = 0; //keeps track of whether basename is found
  int foundv = 0; //keeps track of whether vary is found
  ...
  for (i=0;i<lastop;i++) {
    ...
    switch (op[i].opcode) {
      ...
      case FRAMES:
        printf("Frames: %4.0f", op[i].op.frames.num_frames);
      ...
      case VARY:
        printf("Vary: %0.4f %0.4f %0.4f %0.4f",
  ...
  if (foundv == 1 && foundf == 0) {
    printf("ERROR: VARY found, FRAMES not found. Exiting...\n");
    exit(0);
  }
